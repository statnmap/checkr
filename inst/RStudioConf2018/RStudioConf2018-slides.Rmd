...
tutorial_options(exercise.checker = checktutorials::check_for_learnr)
...
Ordinarily, you would not show students the `checktutorials` statements implementing this behavior. But our purpose here is to introduce `checktutorials`, So here are the statements for the above exercise.

```{r echo = FALSE, comment = "", tidy = FALSE}
print_function_contents(
  trig_2_check)
#  from_file = system.file("learnr_examples/internal-examples.R", package = "checktutorials"))
```
...
## rlang, redpen, and checktutorials

### The foundation

`checktutorials` is based on the `rlang` and `redpen` packages. Two basic technologies:
...
- experience programming with `checktutorials` may lead to better high-level functions for simplifying checking, like `check_blanks`. We need to dig the *Pit of Success* 
