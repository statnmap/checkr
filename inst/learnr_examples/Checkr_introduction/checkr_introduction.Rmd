...
This is a `learnr` tutorial to display some examples of code-checking with the `checktutorials` package.
...
`checktutorials` is a package that provides feedback about the code that is being executed in a `learnr` exercise box.
...
The box below has been instrumented with `checktutorials` statements intended to guide the user to a correct answer. Exactly what "guide the user" means is ambiguous. Play around with various possible answers and see if you agree that the `checktutorials` statement I choose are helpful.
